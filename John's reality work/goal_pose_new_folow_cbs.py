...
def navigation(turtlebot_name, aruco_id, goal_list):
    ...
    while current_position_idx < len(goal_list) and not rospy.is_shutdown():
        ...
        if current_position_idx == 12 and aruco_id == "id100":
            rospy.sleep(6)


        # Check if the goal has been reached
        if check_goal_reached(init_pose, goal_x, goal_y, tolerance=0.3):
...
def get_transformation_matrix(aruco_markers):
    ...
    sim_points = np.float32([
        [-1, -0.5],     # Bottom-left corner in simulation
        [10.5, -0.5],    # Bottom-right corner in simulation
        [-1, 10],    # Top-left corner in simulation
        [10.5, 10]    # Top-right corner in simulation
    ])
...
def main():
    ...
    try:
        ...
        coordinates = read_and_transform_waypoints("./cbs_output_new.yaml", matrix)
    except Exception as e:
